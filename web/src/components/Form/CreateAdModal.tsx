...
export function CreateAdModal() {
    ...
    function handleCreateAd(event: FormEvent){//Por padrão quando um form é enviado o código força que você vá para uma outra página(por isso se usa o action ou o method na tag <form></form>), mas como queremos continuar na mesma usamos um event para isso.
        ...
        //Validação para não salvar o anúncio com os campos vazios
        if(!data.name){
            return;
        }
        ...
        try{
            axios.post(`http://localhost:3333/games/${data.game}/ads`, {
                ...
                weekDays: weekDays.map(Number),//aplica em todos os itens do array a conversão de string para number
    ...
    }//traz os dados do formulário num objeto
                ...
                <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[484px] shadow-lg shadow-black/25">
                    ...
                    <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <select name="game" defaultValue="" className="bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none" id="game">
